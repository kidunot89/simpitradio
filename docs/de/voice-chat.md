...
SimPitRadio tippt, was Sie gesagt haben, in die Chatbox des Spiels. Das hier ergänzt
die andere Hälfte: Die Leute, gegen die Sie fahren, schicken Ihnen auch das *Audio*,
und Sie hören es.

Es ist bewusst kein Discord. Discord gibt es bereits, es funktioniert, und alle sind
schon in einem. Was Discord nicht kann, ist, Sie in einen Raum mit **wer auch immer
in dieser Session ist** zu stecken, ohne das vorher zu verabreden, und die
stummzuschalten, die vier Kilometer entfernt sind.
...
übergibt ihn beim Loslassen an Whisper. Voice verwendet genau diesen Clip weiter: Beim
Loslassen geht er an Whisper *und* an das Relay, und die Gegenstellen spielen ihn ab.
Am Aufnahmepfad ändert sich nichts.
...
werden, er kommt ganz oder gar nicht an, und zwei Leute, die gleichzeitig sprechen,
erzeugen zwei Clips, die sich anstellen, statt einander zu übertönen. Das ist besser
als ein Rennen, nicht schlechter.
...
ein bis zwei Sekunden alt — bei Renntempo hundert Meter, was gegen einen Radius von
200 m die Antwort entscheidet. Der Scoring-Block hat jedes Auto so, wie es *jetzt*
ist, und die Frage ist, wer nahe am Zielauto ist, wenn die Nachricht abgespielt wird.
...
Kilometer weit weg ist, wird schlicht dort gemessen, wo er tatsächlich ist. Das ist
eine Folge davon, die frischere Zahl zu benutzen, kein Sicherheitsmechanismus — ein
Sprecher, den niemand verorten kann, ist weiterhin hörbar, denn Stille, die niemand
erklären kann, ist der schlimmere Ausfall.
...
**Der Shared-Memory-Block sagt es nicht**, und drei plausible Quellen wurden gegen eine
...
- `$rFactor2SMMP_Graphics$` wird veröffentlicht und würde sowohl eine Kameraposition
  als auch eine betrachtete Slot-ID führen — aber LMU befüllt es nie. Der Puffer ist
  ...
  im selben Moment lebendig, das ist also LMUs Entscheidung und keine kaputte
  Installation.

**LMUs eigene HTTP-API sagt es.** `http://127.0.0.1:6397/rest/watch/standings` ist,
was die spieleigenen Overlays lesen, und jeder Eintrag führt `hasFocus` — gesetzt beim
beobachteten Auto, verschieden von `player`, das auf Ihrem eigenen bleibt. Seine
...
Auslösezyklus, die Antwort ist ~16 KB, und eine Diktier-App darf niemals auf ein Spiel
...
Build-Zeit** in [endpoints.py](../src/pitradio/endpoints.py) geschrieben, ein Checkout —
oder ein Fork — hat also überhaupt keine Adresse, und Voice ist schlicht nicht
verfügbar. Das ist ein funktionierender Zustand, kein kaputter: besser, als wenn jeder
Klon der Quelle ein Mikrofon auf einen Server richtet, dessen Besitzer nie zugestimmt
hat, ihn zu tragen.
...
Atlantik; ein für die Gruppe gewähltes Relay nicht. Das ist der ganze Grund, warum Racer
hosten können: nicht Kosten und nicht Dezentralisierung um ihrer selbst willen —
Geografie.
...
Provisionierung — es sind IAM-Schlüssel oder Identity Center SSO — es braucht also einen
eigenen Weg und ist keine Frage davon, einen Knopf hinzuzufügen.
...
2. Der Koordinator wählt das mit dem besten schlechtesten Fall im Raum — er minimiert
   die Latenz des *langsamsten* Racers, nicht den Durchschnitt, denn der Punkt ist, dass
   niemand gestrandet ist.
...
Design — Mumble verkettet Server so — und es ist in einer Hinsicht wirklich eleganter,
weil es die Gruppenentscheidung oben löscht. Es gibt nichts zu vereinbaren, wenn der Raum
sich über jedes Relay erstreckt, der Fehler des geteilten Raums kann also gar nicht
auftreten.

Es ist hier trotzdem der falsche Handel, aus einem Grund: **Wir senden Clips, keinen
Livestream.** Ein Clip wird abgeschickt, nachdem der Sprecher aufgehört hat zu reden, der
Unterschied zwischen 90 ms und 250 ms Routing ist also nichts, was irgendwer wahrnehmen
kann — was der größte Teil des Arguments für Geografie ist und das ganze Argument dafür,
zwei zusätzliche Hops zu zahlen, um es zu verbessern.
...
aus und migriert die verbliebenen Racer — derselbe Mechanismus wie bei der ersten Wahl,
es gibt also keinen separaten Failover-Pfad, den man falsch machen könnte. Genau deshalb
halten Clients die Verbindung zum Koordinator offen: Sie ist das, was überlebt.

Ein Racer, der die Session verlässt, nimmt sein Relay nicht mitten im Rennen mit. Seine
Maschine ist nicht das Relay — ein von ihm provisionierter Droplet ist es — und es den
noch Fahrenden unter den Füßen wegzuziehen wäre der denkbar schlechteste Moment dafür.
...
Voice ist **aus, bis es eingeschaltet wird**, je Profil, und das Fenster sagt, wer Sie
hören kann, bevor es irgendetwas anderes sagt. Eine Diktier-App, die still das Mikrofon
...
geben: Die Taste ist die Einwilligung.
