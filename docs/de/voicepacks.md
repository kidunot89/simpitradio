...
Genau das ist ein Sprachpaket: ein Ordner mit WAV-Dateien, eine je Satz, die der
Ingenieur abspielt, statt über die Windows-Sprachausgabe zu sprechen.

Alles, was er sagt und nicht im Paket steht — Ihr Name, eine Rundenzeit, ein
Fahrer, von dem er nie gehört hat — wird weiterhin von der Windows-Stimme
gesprochen. Ein Paket muss nicht vollständig sein, um etwas zu taugen.
...
Ihnen wichtig sind. Nützlich, wenn Sie sofort etwas Korrektes und Angenehmes
wollen oder lieber nicht 171 Sätze vorlesen, bevor Sie fahren.
...
VITS-Stil: ein Durchlauf von Phonemen zur Wellenform, ohne Sampling-Schleife, die
abschweifen kann. Es kann kein anderes Wort sagen, und das zählt bei diesem
...
mehreren Sitzungen, und ein Paket mit der Hälfte der Sätze funktioniert ab dem
Moment, in dem Sie es speichern.

Ein paar Dinge entscheiden, ob das Ergebnis brauchbar ist:

- **Seitlich am Mund vorbei**, ein paar Fingerbreit entfernt — nicht davor. Ein
...
Lesen Sie sie so, wie ein Ingenieur sie über Funk sagen würde — sachlich,
unaufgeregt, leicht gelangweilt. Der Ingenieur spielt keine Rolle.

## Eine Basis mit Piper erzeugen

Piper läuft offline, aus den Packaging-Skripten heraus statt in der App:
...
verwandeln, und ohne das erzeugt jede Phrase gar kein Audio — gemeldet als
...
Ein Paket ist ein Ordner mit einem Unterordner `voice`, in dem die WAVs liegen —
das Layout, das `crew-chief-autovoicepack` schreibt, also **lässt sich ein
Crew-Chief-Paket direkt einlegen**. Der äußere Ordner ist getrennt, damit ein
Paket eine Lizenz und seine Quellaufnahmen mitführen kann, ohne dass diese für
Sätze gehalten werden.
...
Dateinamen entstehen aus dem Satz, kleingeschrieben, wobei Satzzeichen
**entfallen** statt ersetzt zu werden — „that's enough“ und „thats enough“ sind
also derselbe Clip. Einen Apostroph in ein Trennzeichen zu verwandeln ergäbe
...
Pakete liegen neben Ihrer Konfiguration, in `voices/`, nicht im
...
Ingenieur sagen kann, als CSV, in der Sprache des Ingenieurs — denn ein Paket
wird in der Sprache aufgenommen, in der es gesprochen wird.
...
dasselbe, das Sie für den Sprachchat nutzen, nicht die Ausgabe der Simulation.
