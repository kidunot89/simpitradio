...
odtwarza zamiast mówić przez syntezator Windows.
...
Piper działa offline, ze skryptów pakujących, a nie wewnątrz aplikacji:
...
Pakiet to folder z podfolderem `voice` w środku, w którym leżą pliki WAV — układ,
który zapisuje `crew-chief-autovoicepack`, więc **pakiet z Crew Chiefa wchodzi
wprost**. Folder zewnętrzny jest osobny, żeby pakiet mógł nieść licencję i swoje
nagrania źródłowe, nie myląc ich z frazami.
