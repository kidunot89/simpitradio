...
Dzieli wyzwalacz ze wszystkim innym, co robi SimPitRadio. Przytrzymaj wyzwalacz,
powiedz „Chief, skup się na P3", puść. To trafia do inżyniera zamiast do okna
czatu, i inżynier odpowiada.
...
6. Przytrzymaj wyzwalacz i powiedz **„Chief, skup się na P3"**, żeby być
   mierzonym względem tego, kto jest trzeci.
...
**Powiedz najpierw jego imię.** „Chief, skup się na P3". Imię idzie pierwsze,
fraza zaraz po nim. `hey`, `ok` i `right` są dozwolone przed imieniem.

**Albo powiedz samą frazę**, o ile ma dwa słowa i nie przyjmuje kierowcy.
„trenuj mnie" działa bez niczego z przodu. „analizuj" to jedno słowo — mogłoby
rozpocząć zwyczajne zdanie, a wszystko po nim to imię kierowcy, więc potrzebuje
najpierw imienia inżyniera. Powiedziane samo, *analizuj P2* trafiło do okna
czatu osiem razy w jednej sesji, rozwiązane we wzmiankę o kierowcy i wysłane do
wszystkich.

Powiedzenie samego imienia daje „słucham", tak jak zrobiłoby prawdziwe radio, i
...
zwracania się do niego, a „Bob, skup się na P3" działa dokładnie tak samo
dobrze.
...
Zapowiada samochody obok: „auto z lewej", „auto z prawej", „w środku" dla
jednego po każdej stronie, a potem „czysto", gdy odjadą. Domyślnie wyłączone, i
...
samochodzie po twojej prawej, włącz **Zamień strony spottera** w Profilach, w
ustawieniach wtyczki sesji tej gry. Jedno zaznaczenie, raz.
...
> *Straciłeś kawałek nadwozia. Zjazd w tym okrążeniu.*
...
| | |
| --- | --- |
| **Zjazd w tym okrążeniu** | samochodem nie da się ścigać, tylko doprowadzić go do boksu: zwisający element, przebita opona, utracone koło albo złamany nos. Ilość pozostałego czasu tego nie zmienia — alternatywą jest czarna flaga albo mur |
| **Zjazd, kiedy się da** | warto naprawić. Zawsze na treningu i kwalifikacjach, gdzie postój nic nie kosztuje, a sensem bycia na torze jest działający samochód |
| **Zostań na torze, wytrzymamy** | wyścig, w którym zostało mniej niż jedna piąta. Trzy okrążenia przed metą lepiej dowieźć uszkodzony samochód do flagi niż oddać minutę |
...
Nigdy nie mówi przez trenera. Uszkodzenia są pilne — chcesz wiedzieć, że skrzydło
...
Inżynier utrzymuje **cel**: jednego kierowcę, z którym cię zestawia. Nie musisz
...
- `skup się na {kierowca}` — albo „miej na oku", „obserwuj", „analizuj", „patrz na"
- `domyślny cel` — powrót do samodzielnego wyboru
- `przestań się skupiać` — wyłączone i pozostaje wyłączone, dopóki go nie
  poprosisz z powrotem
...
Karta Status pokazuje, kto jest obserwowany, a `kogo obserwujemy` o to pyta.
...
- **Twój samochód** — `jakie jest moje najlepsze okrążenie`, `jak opony`,
  `jakie są uszkodzenia`, `jak paliwo`, `ile paliwa potrzebuję do końca
  wyścigu, jeśli zjadę do boksu na następnym okrążeniu`
- **Sesja** — `kto ma najszybsze okrążenie`, `kto jest najszybszy`,
  `kto ma najszybszy sektor`, `kto prowadzi`, `kto jest z przodu`
- **Gdzie ucieka czas** — `gdzie jestem wolniejszy`, `gdzie jestem szybszy`, każde
  z nich z „niż {kierowca}" na końcu
...
**Pytanie, na które nie potrafi odpowiedzieć, zostaje poza oknem czatu.** „Kto
jest szybszy?" nie jest frazą, którą zna, i kiedyś przelatywało dalej i szło do
sesji.
...
Imię inżyniera możesz umieścić z każdej strony: „Bono, jak opony" i „jak opony,
Bono" działają oba. To przecinek oznacza je jako imię, więc `skup się na Bono`
nadal celuje w kierowcę o nazwisku Bono.

### Gdzie jestem wolniejszy

Ta, którą warto znać. Zestawia cię z twoim celem **zakręt po zakręcie,
...
> Chief, gdzie jestem wolniejszy
>
> *Zakręt trzy, jesteś wolniejszy na wejściu, dwie dziesiąte.*
>
> *Zakręt siedem, mają lepsze wyjście, dziesiąta.*
...
Inżynier**, a trener wybiera swój na karcie Coaching — to dwa osobne zadania i
...
**Odczytuje czasy okrążeń, ale nigdy nie prowadzi coachingu.** Trener potrzebuje
okrążenia referencyjnego, a dopóki Twój cel nie ukończy jednego czysto, bez
żadnej części w alei serwisowej, nie ma z czym porównywać.

**Prowadzi coaching i przy niektórych zakrętach nic nie mówi.** Te zakręty
kosztują mniej czasu niż próg, i tak jest zaprojektowane. `engineer.coach_threshold`
w `config.json` to ile czasu w połowie zakrętu jest warte przerwania, w
sekundach; obniż, żeby dostać więcej uwag. Nie ma na to kontrolki w oknie.
...
Trzy liczby inżyniera mieszkają w **profilu**, pod ustawieniami wtyczki sesji tej
gry, bo opisują grę, a nie twój gust:
...
pytaniem jest to, czy jest miejsce, by ruszyć, i `rejoin.py` na nie odpowiada,
porównując *czas do bycia bezpiecznym* z *czasem do przyjazdu następnego
samochodu*. Stojący samochód musi odzyskać całe przyspieszenie przed pierwszym
przyjazdem, dlatego odpowiedź zbudowana na trzech sekundach czystego toru
kończy się tym, że kogoś zbiera.
...
jako drugiego umieściłoby „kto ma najszybsze okrążenie" na liście Zachowań, gdzie
...
wpisał w pole ustawień. „Kto ma najszybszy sektor" to fraza; „trzeci w GT3" to
...
wymagając dwóch słów przed otwartym parametrem; tutaj to nie wystarcza, bo „kto
ma najszybsze okrążenie życia, ten jeden" przechodzi to bez trudu i zostałoby
wzięte za pytanie o klasę zwaną „życia, ten jeden" — połykając wiadomość. Ale
...
w samochodzie GT3, pytając „kto ma najszybsze okrążenie". Nazwana klasa, w której
...
go dalej w łańcuchu — inaczej „kto ma najszybsze okrążenie" wciąż byłoby wyjęte z
...
* **„Trzech obok siebie, jesteś z prawej"** było „dwa auta z lewej". Kierowca
  słyszący starą wersję musi wyliczyć, gdzie go to stawia, będąc zajętym; nowa
  mówi wprost, po której stronie nie ma miejsca.
* **„W środku"** było „trzech obok", dla jednego samochodu po każdej stronie.

Powtórzenie mówi `wciąż tam jest` po jednej stronie i `trzymaj swoją linię` po
obu, bo to różne instrukcje — jedna znaczy nie jedź tam, druga znaczy nie ruszaj
się.
...
„Ile paliwa potrzebuję do końca wyścigu, jeśli zjadę do boksu na następnym
okrążeniu" albo „…za pięć okrążeń". **Odpowiedzią jest procent**, bo to jest
liczba na
