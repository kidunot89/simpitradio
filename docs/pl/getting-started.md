...
wybranym. Ustala to trzy rzeczy naraz: okno, model mowy oraz język, w którym
inżynier mówi i słucha. Możesz to później zmienić na karcie Język.
...
**Przycisk kierownicy też działa.** W wierszu **Przycisk kierownicy** naciśnij
*Naciśnij przycisk…* i naciśnij przycisk, który chcesz. SimPitRadio otwiera
kierownicę, nie zabierając jej grze, więc symulator nadal czyta każdy przycisk,
także ten. Przytrzymaj, żeby mówić, dokładnie jak klawiszem. Oba pozostają
uzbrojone naraz, więc możesz przypisać jedno i drugie i używać tego, które jest
bliżej.

Druga droga to własne oprogramowanie kierownicy albo
[JoyToKey](https://joytokey.net/): ustaw `F13` na przycisku i przypisz `F13`
tutaj. Sięgnij po to, jeśli kierownica już korzysta z oprogramowania, któremu
ufasz, albo jeśli SimPitRadio nie może otworzyć urządzenia.

## Ustaw mikrofon

**Dźwięk → Mikrofon.** Wybierz wejście, przytrzymaj wyzwalacz i patrz na pasek
poziomu — pokazuje sygnał *po* wzmocnieniu, czyli to, co model mowy naprawdę
...
4. Po puszczeniu klip trafia do modelu mowy, na Twoim własnym procesorze.
