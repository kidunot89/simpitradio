...
The thing SimPitRadio was built to do. Hold the trigger, say what you want to
say, let go. It appears in the game's chat box, typed out, with both hands
still on the wheel.

Everything else in the app grew out of this. The engineer, the coach and voice
chat share the same trigger and the same clip. Text chat is what happens to the
words when nothing else has claimed them.
...
2. **Recording starts immediately**, before the chat box is opened rather than
   after. Opening it takes a few hundred milliseconds, and anything said in
...
The same trigger sends messages to everyone in your session, so a command the
app invents is a message that silently never arrives. See
[Talking to it](engineer.md#talking-to-it).
...
into the game that meant something else there. Set that one once and forget it.
The app-wide switch still wins: off there is off everywhere.

## Checking a message before it goes out

By default the message is sent as soon as it is typed. The speech model does
mishear things, and in a public session a mistake is everyone's problem, so
each profile has a **Send automatically** toggle.
...
ignore it and read hardware scan codes instead. For those, switch to
...
frame. Typed text is not. It goes through the message queue, where 40ms a
character would make a 200-character message take eight seconds.

## Names and vocabulary

The speech model transcribes what it hears, and drivers' names are what it is
worst at. The session plugin reads who is in your session and feeds those names
in, so "Estre" comes out as "Estre" rather than "Ester".
...
problem is the trigger or the hook rather than transcription.
