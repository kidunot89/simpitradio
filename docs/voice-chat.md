...
SimPitRadio types what you said into the game's chat box. This adds the other
half: the drivers you are racing send you the *audio* as well, and you hear it.

Discord already exists and everybody is already in one. What Discord cannot do
is put you in a room with **whoever is in this session**, without arranging it
beforehand, and quiet the ones who are four kilometres away.
...
it arrives whole or not at all, and two drivers talking at once produce two
clips that queue instead of talking over each other. On a race weekend that is
...
At racing speed that is a hundred metres, and against a 200m radius a hundred
metres decides the answer. The scoring block has every car as of *now*, and the
...
Preferring the local view also means a clip cannot talk its way past the
filter. A client claiming to be alongside you when it is a kilometre away is
measured where it actually is. That falls out of using the fresher number and
is no kind of security mechanism. A speaker nobody can place at all is still
...
**The shared memory block does not say.** Three plausible sources were checked
against a live spectated session and ruled out. Each one looks right and none
of them is:
...
- `$rFactor2SMMP_Graphics$` is published and would carry both a camera position
  and a viewed slot id, and LMU never populates it. The buffer is entirely
  ...
  block was live at the same moment, so this is LMU's choice rather than a
  broken install.

**LMU's own HTTP API does say.** `http://127.0.0.1:6397/rest/watch/standings` is
what the game's own overlays read. Each entry carries `hasFocus`, set on the
car being watched and distinct from `player`, which stays on your own. Its
`slotID` is the same number as `mID` in shared memory, so the two join
directly. It is part of the game rather than any plugin, so it needs nothing
installed.

Read with a short timeout and cached for a second: this runs on the trigger
cycle, the response is ~16KB, and the app must never wait on a game that is
mid-load. **Failures are cached too.** Otherwise a closed game costs a timeout
...
cars in the game, read from the sim, computed locally. It has nothing to do
with where anybody lives, and no physical location is read, derived or
transmitted. Relay *hosting* below talks about distance too, in the network
sense. That is a routing question about servers and is unrelated to who you can
hear.
...
Everyone on the same game server computes the same id without anyone
publishing which server that is. The relay learns a hash and nothing else.
Offline and single-player have no server, so they produce no id and no room,
...
**The relay's code and configuration are not in this repository.** SimPitRadio is
...
`endpoints.py` **at build time**, so a checkout, or a fork, has no address at
all and voice chat is unavailable. That is a working state. It beats every
clone of the source pointing a microphone at a server whose owner never agreed
to carry it.
...
One WebSocket per client, TLS, clips as binary frames with a small header.
That is the whole protocol. TLS because a relay is a stranger's machine and
audio of your voice should not cross it in the clear; WebSocket because it
survives every NAT and corporate firewall that raw UDP does not, and audio
bandwidth for twenty racers pressing a button occasionally is nothing.
...
Relays exist to be *near the people talking*. A grid drawn from three
continents routed through one box in Frankfurt pays the Atlantic twice on every
clip; a relay chosen for the group does not. Geography is the whole reason
racers can host. Cost is not, and neither is decentralisation for its own
sake.
...
provisioning, only IAM keys or Identity Center SSO, so it needs a path of its
...
2. The coordinator picks the one with the best worst case across the room. It
   minimises the *slowest* racer's latency rather than the average, because the
   point is that nobody is stranded.
...
*it*, and have the relays forward clips to each other. It is a real design, and
Mumble links servers this way. It is more elegant in one respect, because it
deletes the group decision above. There is nothing to agree on if the room
spans every relay, so the split-room failure cannot happen at all.

It is still the wrong trade here, for one reason: **we send clips, not a live
stream.** A clip is dispatched after the speaker has stopped talking, so nobody
can perceive the difference between 90ms and 250ms of routing. That difference
is most of the argument for geography, and all of the argument for paying two
extra hops to improve it.
...
If SimPitRadio ever does go live-streaming, this reverses and bridging becomes the
...
room, notices the relay stop answering, re-runs the choice over what is left
and migrates the remaining racers. That is the same mechanism as the initial
pick, so there is no separate failover path to get wrong. Clients keep the
coordinator connection open for exactly this reason: it is the thing that
survives.

A racer leaving the session does not take their relay with them mid-race. Their
machine is not the relay; a droplet they provisioned is. Pulling that out from
under the drivers still on track would be the worst possible moment for it.
...
The idle timeout matters as much as the explicit signal. A client that
crashes, alt-tabs into oblivion or loses its network never sends anything, so
nothing may depend on it doing so.

## Consent

Voice chat is **off until switched on**, and the Voice tab says who can hear
you before it says anything else. An app that quietly opened the
microphone to twenty strangers would be a betrayal, however good the feature
is.
