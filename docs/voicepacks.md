...
pack is that: a folder of WAV files, one folder per phrase, that the engineer
plays instead of speaking through the Windows voice. Several takes of a phrase
can sit in its folder, and the engineer picks one at random, so the same call
twice in a stint does not sound like a machine repeating itself.

Anything it says that is not in the pack is still spoken by the Windows voice:
your name, a lap time, a driver it has never heard of. A pack does not have to
be complete to be worth having.
...
about. Reach for it if you want something correct and pleasant today, or would
...
there is almost nothing constraining that decision. Piper is a VITS-style
model, one forward pass from phonemes to waveform, with no sampling loop to
wander off. It cannot say a different word, which on this inventory matters
more than timbre does.
...
It is less work than it sounds. The whole inventory is **about forty minutes
at three takes each**, and it resumes, so it can be done in sittings. A pack
covering half the phrases works from the moment you save it.

A few things decide whether the result is usable:

- **Off to the side of your mouth**, a couple of finger widths away, rather
  than in front of it. A boom mic directly in the airflow clips on every *p*
  and *b*, and clipping cannot be undone afterwards.
...
Read them the way an engineer says them on the radio: flat, unhurried,
slightly bored. The engineer is not performing.

## Generating a base with Piper

Piper runs offline, from the packaging scripts rather than inside the app. The
paths below are relative to `apps/client` in a source checkout:
...
without it every phrase produces no audio. What it reports is
...
A pack is a folder with a `voice` subfolder inside it, and inside that one
folder per phrase holding its takes. That is the layout
`crew-chief-autovoicepack` writes, so **a Crew Chief pack drops straight in**.
The outer folder is separate so a pack can carry a licence and its source
...
The folder a WAV sits in is the phrase, so the depth between `voice` and that
folder is free. The recorder writes `pitradio/`; Crew Chief writes a folder per
category. Either is read the same way.
...
Folder names come from the phrase, lowercased, with punctuation **dropped**
rather than replaced, so "that's enough" and "thats enough" are the same clip.
...
`voices/`, rather than under the install directory. An update replaces the
install directory wholesale, and a pack is a lot of audio you chose to put
there. **Settings → Voice → Open voice pack folder** opens it.
...
say, as a CSV, in the engineer's own language. A pack is recorded in the
...
one you use for voice chat rather than the sim's output.
