...
Aquello para lo que se construyó SimPitRadio. Mantén el disparador, di lo que
quieras decir, suéltalo. Aparece en el chat del juego, escrito, con las dos
manos todavía en el volante.

Todo lo demás en la aplicación creció a partir de esto. El ingeniero, el
entrenador y el chat de voz comparten el mismo disparador y el mismo clip. El
...
estrecha. El mismo disparador manda mensajes a todos los de tu sesión, así que
una orden que la aplicación se invente es un mensaje que en silencio nunca
llega. Ver [Hablar con él](engineer.md#hablar-con-él).
...
Por defecto el mensaje se envía en cuanto se escribe. El modelo de voz entiende
mal cosas, y en una sesión pública un error es problema de todos, así que cada
perfil tiene un conmutador **Enviar automáticamente**.
...
cualquier alfabeto. Algunos juegos lo ignoran y leen códigos de exploración del
hardware en su lugar. Para esos, cambia a *scancode*, que escribe como si las
teclas se hubieran pulsado físicamente.
...
una vez por fotograma. El texto escrito no. Pasa por la cola de mensajes, donde
40 ms por carácter harían que un mensaje de 200 caracteres tardara ocho
segundos.

## Nombres y vocabulario

El modelo de voz transcribe lo que oye, y los nombres de los pilotos son lo que
peor se le da. El plugin de sesión lee quién está en tu sesión y le pasa esos
...
mismo y cuándo se vio el disparador por última vez. Si *Último disparo* no se
actualiza nunca, el problema es el disparador o el hook, y no la transcripción.
