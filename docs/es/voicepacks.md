...
voz es exactamente eso: una carpeta de archivos WAV, una carpeta por frase, que
el ingeniero reproduce en lugar de hablar con la voz de Windows. En la carpeta
de una frase pueden caber varias tomas, y el ingeniero elige una al azar, así
que el mismo aviso repetido en una tanda no suena a máquina repitiéndose.

Todo lo que diga y no esté en el pack lo sigue diciendo la voz de Windows: tu
nombre, un tiempo por vuelta, un piloto del que nunca ha oído hablar. Un pack no
tiene que estar completo para merecer la pena.
...
importen. Recurre a esto si quieres algo correcto y agradable hoy mismo, o si
prefieres no leer 171 frases antes de conducir.
...
sesiones. Un pack que cubra la mitad de las frases funciona desde el momento en
que lo guardas.
...
Léelas como las dice un ingeniero por la radio: planas, sin prisa, ligeramente
aburridas. El ingeniero no está actuando.

## Generar una base con Piper

Piper se ejecuta sin conexión, desde los scripts de empaquetado y no dentro de
la aplicación. Las rutas de abajo son relativas a `apps/client` en un checkout
del código fuente:
...
sin él cada frase no produce audio alguno. Lo que se informa es
...
Un pack es una carpeta con una subcarpeta `voice` dentro, y dentro de esa, una
carpeta por frase con sus tomas. Ese es el diseño que escribe
`crew-chief-autovoicepack`, así que **un pack de Crew Chief entra
directamente**. La carpeta exterior está separada para que un pack pueda
llevar una licencia y sus grabaciones originales sin que se confundan con
frases.
...
La carpeta en la que está un WAV es la frase, así que la profundidad entre
`voice` y esa carpeta es libre. El grabador escribe `pitradio/`; Crew Chief
escribe una carpeta por categoría. Las dos se leen igual.
...
Los nombres de carpeta salen de la frase, en minúsculas, con la puntuación
**eliminada** en vez de sustituida, así que «that's enough» y «thats enough» son
el mismo clip. Convertir un apóstrofo en separador daría `that_s_enough`, y un
pack grabado contra una de las dos grafías fallaría la otra en silencio.

## Dónde viven los packs

**Ajustes → Voz** es donde están todos listados: lo instalado, lo incluido y lo
descargable. Los packs viven junto a tu configuración, en `voices/`, y no bajo
el directorio de instalación. Una actualización reemplaza ese directorio por
...
ingeniero puede decir, en CSV, en el idioma del propio ingeniero. Un pack se
graba en el idioma en que se va a hablar.
...
auriculares, los mismos que usas para el chat de voz y no la salida del
simulador.
