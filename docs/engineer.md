...
It shares the push-to-talk key with everything else SimPitRadio does. Hold the
...
**Said when it changes, not while it lasts.** A driver carrying a dented left
rear for half an hour does not need reminding of it each time round, so it
speaks on the moment it gets worse and then stays quiet. Every part of the
reading is watched, not just the worst of them — a second thing coming off a
car that was already dented is news even though the severity has not moved.
...
fix it. So it names the place — the nose, the tail, down the left side — and
then gives one of three answers:
...
It never speaks over the coach. Damage is urgent — you want to know the wing
has gone before the next corner rather than after it — but a critique you asked
for is not worth cutting for a car that will still be broken in four seconds.
The spotter is the only call that talks over anything.
...
the coach picks its own on the Coaching tab — the two are separate jobs and a
driver can reasonably want to hear which of them is talking.

**Installing, recording and removing packs is Settings → Voice.** A pack is
something the application holds; which of them a persona speaks from is a
setting on that persona.
...
can be dropped straight in. To generate one for SimPitRadio's phrases rather than
Crew Chief's:
...
The engineer runs on **its own thread**, separate from the four SimPitRadio already
...
SimPitRadio owns a copy — so unlike the rest, it will not get checked against the
...
zone: SimPitRadio had a rule saying a car much slower than you was a hazard, and
...
Distinct from behaviours, and the distinction is not bookkeeping. A behaviour
is something the engineer *keeps doing* — see [What it tells
you](#what-it-tells-you) — and it carries a repeat interval, because a car
alongside stops being there without anything happening. A question has an
answer, and when the answer has been given there is nothing running. Modelling
one as the other would put "who has the fastest lap" in the Behaviours list,
where every entry has a repeat interval, and there is no such thing as
answering a question again every 1.2 seconds.
...
than counting words. `phrases.MIN_BARE_WORDS` protects the spoken commands by
