...
1. **A tecla é engolida.** O jogo nunca vê o acionamento, por isso pode ser uma
...
uma corrida quando uma sessão se torna pública. Desligado, o acionamento fica só
...
Por predefinição a mensagem é enviada assim que é escrita. O modelo de voz erra de vez em quando, e
...
acionamento decide então o que lhe acontece, sem largar o volante:
...
Se tiveres botões a mais, **Definições → Acionamento** também atribui teclas
...
O modelo de voz transcreve o que ouve, e os nomes dos pilotos são exatamente aquilo em
...
momento e quando o acionamento foi visto pela última vez. Se *Último acionamento*
