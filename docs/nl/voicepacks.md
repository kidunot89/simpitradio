...
stempakket is precies dat: een map met WAV-bestanden, één per zin, die de
engineer afspeelt in plaats van via de Windows-synthesizer te spreken.
...
Piper draait offline, vanuit de packagingscripts en niet binnen de app:
...
Een pakket is een map met daarin een submap `voice` die de WAV's bevat — de
indeling die `crew-chief-autovoicepack` schrijft, dus **een Crew Chief-pakket
past er zo in**. De buitenste map staat apart zodat een pakket een licentie en
zijn bronopnames kan meedragen zonder dat die voor zinnen worden aangezien.
...
zijn — dezelfde die je voor spraakchat gebruikt, niet de uitvoer van de sim.
