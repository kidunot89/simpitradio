...
par phrase, que l'ingénieur joue au lieu de parler via le synthétiseur de
Windows.

Tout ce qu'il dit et qui n'est pas dans le pack — votre nom, un temps au tour, un
pilote dont il n'a jamais entendu parler — reste prononcé par la voix Windows. Un
...
comptent pour vous. Utile si vous voulez tout de suite quelque chose de correct
et d'agréable, ou si vous préférez ne pas lire 171 phrases avant de rouler.
...
d'audio de référence propre. Repassée dans le moteur de reconnaissance vocale de
...
d'échantillonnage qui puisse s'égarer. Il ne peut pas dire un autre mot, et sur
cet inventaire cela compte plus que le timbre.
...
en étiez — cela peut donc se faire en plusieurs séances, et un pack couvrant la
moitié des phrases fonctionne dès l'instant où vous l'enregistrez.

Quelques points décident si le résultat est utilisable :

- **Sur le côté de la bouche**, à deux doigts de distance, pas devant. Un micro
  casque directement dans le flux d'air sature à chaque *p* et *b*, et la
  saturation ne se rattrape pas après coup.
...
Lisez-les comme un ingénieur les dirait à la radio : neutre, sans hâte, un peu
...
en phonèmes, et sans lui chaque phrase ne produit aucun audio — signalé par
`wave.Error: # channels not specified`, qui est le fichier de sortie vide et
...
Un pack est un dossier contenant un sous-dossier `voice` qui abrite les WAV — la
disposition qu'écrit `crew-chief-autovoicepack`, donc **un pack Crew Chief
s'insère directement**. Le dossier extérieur est distinct pour qu'un pack puisse
porter une licence et ses enregistrements sources sans que ceux-ci soient pris
pour des phrases.
...
Les noms de fichiers viennent de la phrase, en minuscules, la ponctuation étant
**supprimée** plutôt que remplacée — ainsi « that's enough » et « thats enough »
sont le même clip. Transformer une apostrophe en séparateur donnerait
...
votre configuration, dans `voices/`, pas sous le dossier d'installation — une
mise à jour remplace ce dossier en entier, et un pack représente beaucoup d'audio
que vous avez choisi d'y mettre. **Réglages → Voix → Ouvrir le dossier des packs
de voix** l'ouvre.
...
l'ingénieur peut dire, en CSV, dans la langue de l'ingénieur — parce qu'un pack
...
casque — le même que pour le chat vocal, pas la sortie du simulateur.
