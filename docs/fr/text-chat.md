...
voulez dire, relâchez. Cela apparaît dans le chat du jeu, tapé, les deux mains
...
2. **L'enregistrement démarre immédiatement**, avant l'ouverture du chat du
   jeu plutôt qu'après. L'ouvrir prend quelques centaines de millisecondes, et
   tout ce qui serait dit pendant ce temps serait perdu.
3. **Les touches de chat sont envoyées** pour ouvrir le chat du jeu, et
   l'application attend `pre_delay_ms` qu'il prenne le focus.
...
à la place et rien n'est tapé. Cette décision est volontairement étroite. La
même touche envoie des messages à tout le monde dans votre session, donc une
commande que l'application invente est un message qui n'arrive jamais, en
silence. Voir [Lui parler](engineer.md#lui-parler).

## L'éteindre

**Chat texte → Envoi vers le jeu** est l'interrupteur vers lequel se tourner en
...
Il existe un second interrupteur par jeu, sous Profils. « Ce jeu a-t-il un chat »
est un fait sur le jeu plutôt qu'une décision à prendre à chaque session.
Assetto Corsa hors ligne n'a pas de chat à ouvrir, donc chaque pression envoyait
au jeu un Entrée qui y voulait dire autre chose. Réglez-le une fois et oubliez-le.
L'interrupteur global l'emporte toujours : éteint là, éteint partout.

## Vérifier un message avant qu'il parte

Par défaut le message est envoyé dès qu'il est tapé. Le modèle vocal se trompe
parfois, et dans une session publique une erreur est le problème de tout le
monde, donc chaque profil a une bascule **Envoyer automatiquement**.

Désactivée, le message est saisi dans le chat du jeu et y reste. Votre
...
(`pre_delay_ms`). Le chat du jeu a besoin de quelques images pour s'ouvrir et
...
| **Touches d'annulation** | Ce qui ferme le chat sans envoyer, pour effacer un message en attente |
...
Le modèle vocal transcrit ce qu'il entend, et les noms de pilotes sont ce qu'il
réussit le moins bien. Le plugin de session lit qui est dans votre session et
lui fournit ces noms, si bien que « Estre » ressort en « Estre » et non en
« Ester ».
...
déclenchement* ne bouge jamais, le problème vient du déclencheur ou du hook,
pas de la transcription.
...
**Vérifiez que le jeu n'ignore pas l'unicode.** Si le chat du jeu s'ouvre et
