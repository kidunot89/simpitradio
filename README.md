...
with your wheel's own software or JoyToKey, and PitRadio sees it as an ordinary
key.
...
catalogue and extracted to `src/pitradio/locale/template.json`, so a language
...
PitRadio type into a game's chat box; a plugin is what lets the engineer know
...
PitRadio changes.** Everything described below runs on your own machine, works
...
with **[JoyToKey](https://joytokey.net/)**, and PitRadio sees an ordinary
keypress. That is the route to use if your wheel came with software you already
run, which most do.

PitRadio can also **bind a wheel button directly**, with nothing in between —
see below. Either way works; the direct route needs no third-party software,
and the F13 route works with any device at all, including one PitRadio cannot
open.
...
button with **[JoyToKey](https://joytokey.net/)** and PitRadio sees an ordinary
